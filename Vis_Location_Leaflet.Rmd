...
params:
  point_data:
    label: "Input points:"
    value: "C:/Users/ronja/Documents/EAGLE/Svalbard/larissa_sos/faeces_sampling_2023_2024_all_data_continuously_updated.csv"
    input: file
  northing_column:
...
data_clean <- data %>%
  filter(
    !is.na(.data[[params$easting_column]]) & 
    !is.na(.data[[params$northing_column]]) & 
    .data[[params$easting_column]] >= valid_range$easting[1] & 
    .data[[params$easting_column]] <= valid_range$easting[2] &
    .data[[params$northing_column]] >= valid_range$northing[1] & 
    .data[[params$northing_column]] <= valid_range$northing[2]
  )

# Step 3: Create an sf object with UTM coordinates (EPSG:25833)
data_sf <- st_as_sf(data_clean, coords = c(params$easting_column, params$northing_column), crs = 25833)
...
for (i in 1:length(lng)) {
  reindeer_map <- reindeer_map %>%
    addCircleMarkers(
      ...
      popup = paste(
        "Infos:", 
        "\nDate:", data_projected[[params$date_column]][i], 
        "\nSex:", data_projected[[params$sex_column]][i], 
        "\nAge:", data_projected[[params$age_column]][i]
      )
    )
}
  ...
  htmltools::HTML(paste0(
    "<div style='font-family: Arial, sans-serif; padding: 6px;'>
       ...
       <div style='padding: 6px; border: 1px solid #ddd; border-top: none;
         ...
         <b>Date: </b>", data_projected[[params$date_column]][i], "<br/>
         <b>Sex: </b>", data_projected[[params$sex_column]][i], "<br/>
         <b>Age: </b>", data_projected[[params$age_column]][i], "<br/>
         <b>Coordinates:</b><br/>
...
for (i in 1:length(lng)) {
  current_sex <- data_projected[[params$sex_column]][i]
...
for (i in 1:length(lng)) {
  current_sex <- data_projected[[params$sex_column]][i]  # Fixed to use params
  marker_color <- ifelse(current_sex %in% names(sex_colors),
                        sex_colors[current_sex],
                        "#95a5a6")
